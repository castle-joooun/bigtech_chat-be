"""
메시지 API 엔드포인트 (Message API Endpoints)

================================================================================
아키텍처 개요 (Architecture Overview)
================================================================================
채팅 메시지 관련 작업을 처리하는 FastAPI 라우터입니다.
메시지 전송, 조회, 읽음 처리, 실시간 스트리밍을 제공합니다.

메시지 저장 구조:
    ├── MongoDB: 메시지 본문, 읽음 상태
    ├── Redis: 최근 메시지 캐시
    └── Kafka: 실시간 메시지 이벤트

실시간 메시지 흐름:
    클라이언트 → POST /messages/{room_id} → MongoDB 저장
                        ↓
                 Redis 캐시 업데이트
                        ↓
                 Kafka 이벤트 발행
                        ↓
              GET /messages/stream/{room_id} SSE로 수신

================================================================================
API 엔드포인트 (API Endpoints)
================================================================================
- POST /messages/{room_id}: 메시지 전송
- GET /messages/{room_id}: 메시지 목록 조회 (페이지네이션)
- POST /messages/read: 메시지 읽음 표시
- GET /messages/room/{room_id}/unread-count: 읽지 않은 메시지 수
- GET /messages/stream/{room_id}: 실시간 메시지 스트리밍 (SSE + Kafka)
...
from legacy.app.schemas.message import (
    MessageCreate, MessageResponse, MessageListResponse, MessageUpdate,
    MessageDeleteRequest, MessageImageUpload, MessageReadRequest, MessageReadResponse,
    MessageSearchRequest, MessageSearchResponse, MessageStatsResponse
)
from legacy.app.api.auth import get_current_user
from legacy.app.core.errors import ResourceNotFoundException, BusinessLogicException, AuthorizationException
from legacy.app.core.validators import Validator
from legacy.app.services import message_service, chat_room_service
...
async def send_message(
    ...
    # 새 메시지를 Redis 캐시에 추가
    try:
        from legacy.app.services.message_cache_service import cache_new_message
        cache_dict = {
            "id": str(message.id),
            "room_id": room_id,
    ...
    try:
        ...
        from legacy.app.domain.events.message_events import MessageSent
...
async def get_messages(
    ...
    try:
        from legacy.app.services.message_cache_service import get_cached_room_messages
...
async def stream_room_messages(
    ...
    async def event_generator():
        ...
        try:
            ...
            from legacy.app.infrastructure.kafka.config import kafka_config
            import asyncio
