"""
온라인 상태 API 엔드포인트 (Online Status API Endpoints)

================================================================================
아키텍처 개요 (Architecture Overview)
================================================================================
사용자 온라인 상태 조회 및 실시간 스트리밍을 처리하는 FastAPI 라우터입니다.
Redis에 저장된 온라인 상태를 조회하고, Kafka를 통해 실시간 업데이트를 수신합니다.

온라인 상태 흐름:
    로그인 → Redis에 온라인 상태 저장 → Kafka 이벤트 발행
                        ↓
    Heartbeat (30초마다) → TTL 연장
                        ↓
    TTL 만료 → HeartbeatMonitor 감지 → 오프라인 처리 → Kafka 이벤트

클라이언트 수신:
    GET /online-status/stream → SSE 연결 → Kafka 이벤트 수신 → 실시간 상태 업데이트

================================================================================
API 엔드포인트 (API Endpoints)
================================================================================
- GET /online-status/user/{user_id}: 특정 사용자 온라인 상태 조회
- POST /online-status/heartbeat: Heartbeat 전송 (클라이언트가 주기적 호출)
- GET /online-status/friends: 친구들의 온라인 상태 일괄 조회
- GET /online-status/stream: 친구 온라인 상태 실시간 스트리밍 (SSE)
...
>>>
>>> # 친구 상태 스트리밍 (EventSource)
>>> const es = new EventSource('/online-status/stream');
