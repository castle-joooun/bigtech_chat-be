"""
채팅방 서비스 (Chat Room Service)

================================================================================
아키텍처 개요 (Architecture Overview)
================================================================================
채팅방 관련 데이터베이스 작업을 처리하는 서비스 레이어입니다.
MySQL(ChatRoom, RoomMember)과 MongoDB(Message, MessageReadStatus) 양쪽에 접근합니다.

계층 구조:
    API Layer (chat_room.py)
         ↓
    Service Layer (chat_room_service.py) ← 현재 모듈
         ↓
    ├── MySQL (ChatRoom, RoomMember)
    └── MongoDB (Message, MessageReadStatus)

주요 기능:
    ├── CRUD: create_chat_room, find_chat_room_by_id
    ├── 조회: get_user_chat_rooms, find_existing_chat_room
    ├── 메시지: get_last_message, get_unread_count
    └── 헬퍼: get_other_participant_id, is_user_in_chat_room

================================================================================
디자인 패턴 (Design Patterns)
================================================================================
1. Repository Pattern
   - 데이터 접근 로직을 서비스 레이어로 캡슐화
   - API 레이어는 DB 구조를 알 필요 없음

2. Unit of Work Pattern
   - AsyncSession을 통한 트랜잭션 관리
   - 커밋/롤백은 호출자가 제어

================================================================================
사용 예시 (Usage Examples)
================================================================================
>>> from legacy.app.services import chat_room_service
>>>
>>> # 채팅방 조회 또는 생성
>>> existing = await chat_room_service.find_existing_chat_room(db, user1, user2)
>>> if not existing:
...     room = await chat_room_service.create_chat_room(db, user1, user2)
>>>
>>> # 마지막 메시지 조회
>>> last_msg = await chat_room_service.get_last_message(room_id=10)
"""
...
from legacy.app.models.chat_rooms import ChatRoom
from legacy.app.models.room_members import RoomMember
from legacy.app.models.users import User
from legacy.app.models.messages import Message, MessageReadStatus


# =============================================================================
# 채팅방 CRUD 작업 (Chat Room CRUD Operations)
# =============================================================================
# 채팅방의 생성, 조회, 수정 작업을 처리합니다.
# 모든 함수는 AsyncSession을 인자로 받아 트랜잭션을 공유합니다.
# =============================================================================
...
async def find_chat_room_by_id(db: AsyncSession, room_id: int) -> Optional[ChatRoom]:
    ...
    Returns:
        Optional[ChatRoom]: 채팅방 객체 또는 None
    """
...
async def find_existing_chat_room(db: AsyncSession, user1_id: int, user2_id: int) -> Optional[ChatRoom]:
    ...
    user_1_id와 user_2_id의 순서에 관계없이 채팅방을 찾습니다.

    Args:
        db: SQLAlchemy 비동기 세션
        user1_id: 첫 번째 사용자 ID
        user2_id: 두 번째 사용자 ID

    Returns:
        Optional[ChatRoom]: 기존 채팅방 또는 None
    """
    result = await db.execute(
...
async def create_chat_room(db: AsyncSession, user1_id: int, user2_id: int) -> ChatRoom:
    ...
    중복 방지를 위해 user_id가 작은 쪽을 user_1_id로 설정합니다.
    ...
    Note:
        호출 전 find_existing_chat_room()으로 중복 확인 권장
    """
