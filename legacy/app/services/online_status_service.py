"""
온라인 상태 관리 서비스 (Online Status Service)

================================================================================
아키텍처 개요 (Architecture Overview)
================================================================================
Redis를 사용하여 사용자의 온라인 상태를 실시간으로 관리합니다.
TTL(Time To Live) 기반으로 자동 만료되며, Heartbeat로 상태를 유지합니다.

Redis 키 구조:
    ├── user:online:{user_id}     # 온라인 상태 데이터 (TTL: 60초)
    ├── user:last_seen:{user_id}  # 마지막 접속 시간 (TTL: 7일)
    ├── online_users              # 온라인 사용자 ID Set
    └── user:websocket:{user_id}  # WebSocket 세션 ID

상태 전이:
    로그인 → set_user_online() → TTL 시작
                   ↓
    Heartbeat → update_user_activity() → TTL 연장
                   ↓ (TTL 만료)
    자동 오프라인 → HeartbeatMonitor 감지 → Kafka 이벤트 발행

================================================================================
Kafka 이벤트 통합 (Kafka Event Integration)
================================================================================
온라인 상태 변경 시 Kafka로 이벤트를 발행합니다:
    - Topic: user.online_status
    - Event: UserOnlineStatusChanged

클라이언트는 SSE를 통해 이 이벤트를 실시간으로 수신합니다.

================================================================================
사용 예시 (Usage Examples)
================================================================================
>>> from legacy.app.services.online_status_service import OnlineStatusService
>>>
>>> # 사용자 온라인 설정
>>> await OnlineStatusService.set_user_online(user_id=123, session_id="sess-abc")
>>>
>>> # Heartbeat (30초마다 호출)
>>> await OnlineStatusService.update_user_activity(user_id=123)
>>>
>>> # 친구들의 온라인 상태 조회
>>> statuses = await OnlineStatusService.get_users_status([1, 2, 3])
"""
...
# =============================================================================
# Redis 키 패턴 (Redis Key Patterns)
# =============================================================================
# 일관된 키 네이밍으로 Redis 데이터를 관리합니다.
# {user_id} 플레이스홀더는 실제 사용자 ID로 대체됩니다.
# =============================================================================
USER_ONLINE_KEY = "user:online:{user_id}"      # 온라인 상태 JSON 데이터
USER_LAST_SEEN_KEY = "user:last_seen:{user_id}"  # 마지막 접속 시간 (ISO 형식)
ONLINE_USERS_SET = "online_users"              # 온라인 사용자 ID Set
USER_WEBSOCKET_KEY = "user:websocket:{user_id}"  # WebSocket 세션 ID

# =============================================================================
# TTL 설정 (Time To Live Configuration)
# =============================================================================
# TTL 만료 시 자동으로 오프라인 처리됩니다.
# Heartbeat로 TTL을 연장하여 온라인 상태를 유지합니다.
# =============================================================================
ONLINE_STATUS_TTL = 60  # 1분 (테스트용 - 프로덕션에서는 300초/5분 권장)
LAST_SEEN_TTL = 86400 * 7  # 7일 (마지막 접속 시간 보관 기간)
...
class OnlineStatusService:
    ...
    Redis를 사용하여 사용자의 온라인 상태를 실시간으로 관리합니다.
    모든 메소드는 static method로 구현되어 상태를 갖지 않습니다.

    주요 기능:
        - set_user_online(): 사용자 온라인 상태 설정
        - set_user_offline(): 사용자 오프라인 상태 설정
        - is_user_online(): 온라인 여부 확인
        - update_user_activity(): Heartbeat 처리
        - get_users_status(): 여러 사용자 상태 일괄 조회
    """
    ...
    async def set_user_online(user_id: int, session_id: str = None) -> bool:
        ...
        try:
            ...
            try:
                from legacy.app.infrastructure.kafka.producer import get_event_producer
                from legacy.app.domain.events.user_events import UserOnlineStatusChanged
