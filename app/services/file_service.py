...
MESSAGE_IMAGES_DIR = UPLOAD_DIR / "message_images"
...
def ensure_upload_directories():
    ...
    MESSAGE_IMAGES_DIR.mkdir(exist_ok=True)
...
async def save_message_image(file: UploadFile, user_id: int) -> str:
    """메시지 이미지 저장"""
    ...
    # 고유한 파일명 생성
    unique_filename = generate_unique_filename(file.filename)
    file_path = MESSAGE_IMAGES_DIR / unique_filename
    ...
    try:
        ...
        # 상대 URL 반환 (서버의 기본 URL + 파일 경로)
        return f"/uploads/message_images/{unique_filename}"
...
async def cleanup_orphaned_files() -> int:
    """고아 파일들 정리 (사용되지 않는 파일들)"""
    # TODO: 데이터베이스와 비교하여 사용되지 않는 파일들 삭제
    # 현재는 기본 구현만 제공
    return 0
...
def get_upload_stats() -> dict:
    ...
    try:
        profile_images_count = len(list(PROFILE_IMAGES_DIR.glob("*")))
        profile_images_size = sum(f.stat().st_size for f in PROFILE_IMAGES_DIR.glob("*"))

        return {
            "profile_images_count": profile_images_count,
            "profile_images_size": profile_images_size,
            "total_files": profile_images_count,
            "total_size": profile_images_size
        }
    except Exception:
        return {
            "profile_images_count": 0,
            "profile_images_size": 0,
            "total_files": 0,
            "total_size": 0
        }